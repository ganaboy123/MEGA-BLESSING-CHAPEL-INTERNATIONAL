...
const initAudio=()=>{if(!d.audioShell||!d.audioEl||!d.audioRange||!d.audioTime)return;st.audio=d.audioEl;const upd=()=>{if(!st.audio)return;const cur=Number.isFinite(st.audio.currentTime)?st.audio.currentTime:0;const dur=Number.isFinite(st.audio.duration)?st.audio.duration:0;const pct=dur>0?cur/dur*100:0;d.audioRange.value=String(Math.max(0,Math.min(100,pct)));const tf=s=>`${String(Math.floor(s/60)).padStart(2,'0')}:${String(Math.floor(s%60)).padStart(2,'0')}`;d.audioTime.textContent=`${tf(cur)} / ${tf(dur)}`;};d.audioRange.addEventListener('input',()=>{if(!st.audio||!Number.isFinite(st.audio.duration)||st.audio.duration<=0)return;st.audio.currentTime=Number(d.audioRange.value)/100*st.audio.duration;});st.audio.addEventListener('timeupdate',upd);st.audio.addEventListener('loadedmetadata',upd);st.audio.addEventListener('ended',upd);document.addEventListener('click',e=>{const b=e.target instanceof Element?e.target.closest('.play-button[data-audio-src]'):null;if(!b||!st.audio)return;const src=b.getAttribute('data-audio-src');if(!src)return;if(st.audio.getAttribute('src')!==src)st.audio.src=src;st.audio.play().catch(()=>{});if(d.audioTitle)d.audioTitle.textContent=b.getAttribute('data-audio-title')||'Sermon Audio';if(d.audioSpeaker)d.audioSpeaker.textContent=b.getAttribute('data-audio-speaker')||'';});};
...
const init=()=>{initChrome();initNav();initScroll();initReveal();initTransitions();initModal();initBlogModal();initPager();initAudio();initGive();initContact();initPrayer();initNews();initData();refreshCountdowns();ensureTicker();d.body.classList.add('loaded','is-ready');if(document.readyState==='complete')hideLoader();else addEventListener('load',hideLoader,{once:true});};
init();
